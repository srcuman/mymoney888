...
app.put('/api/books/:bookId/accounts/:accountId', verifyToken, async (req, res) => {
  try {
    ...
    await connection.query(
      ...
      [name, type, balance, description, accountId, bookId]
    );
    connection.release();
...
app.get('/api/books/:bookId/transactions', verifyToken, async (req, res) => {
  try {
    ...
    let query = `
      SELECT t.*, a.name as account_name, cat.name as category_name, m.name as merchant_name
      FROM transactions t
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO transactions 
       (book_id, user_id, type, amount, account_id, to_account_id, category_id, merchant_id, date, remark, is_installment, installment_id, installment_period, installment_total_periods)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [bookId, req.userId, type, amount, accountId, toAccountId || null, categoryId || null, merchantId || null, date, remark || null, isInstallment || false, installmentId || null, installmentPeriod || null, installmentTotalPeriods || null]
    );
...
// 更新交易
app.put('/api/books/:bookId/transactions/:transactionId', verifyToken, async (req, res) => {
  try {
    const { bookId, transactionId } = req.params;
    const { type, amount, accountId, toAccountId, categoryId, merchantId, date, remark } = req.body;
    const connection = await pool.getConnection();
    
    await connection.query(
      `UPDATE transactions 
       SET type = ?, amount = ?, account_id = ?, to_account_id = ?, category_id = ?, merchant_id = ?, date = ?, remark = ?
       WHERE id = ? AND book_id = ?`,
      [type, amount, accountId, toAccountId || null, categoryId || null, merchantId || null, date, remark, transactionId, bookId]
    );
    connection.release();
    res.json({ message: '更新成功' });
  } catch (error) {
    console.error('更新交易失败:', error);
    res.status(500).json({ error: '更新交易失败: ' + error.message });
  }
});
  ...
  try {
    ...
    const [incomeByCategory] = await connection.query(
      `SELECT c.name, SUM(t.amount) as total
       ...
       WHERE t.book_id = ? AND t.type = 'income'${dateFilter}
    ...
    const [expenseByCategory] = await connection.query(
      `SELECT c.name, SUM(t.amount) as total
       FROM transactions t
       JOIN categories c ON t.category_id = c.id
       WHERE t.book_id = ? AND t.type = 'expense'${dateFilter}
       GROUP BY c.id, c.name
       ORDER BY total DESC`,
      params
    );
    ...
    const [totals] = await connection.query(
      `SELECT 
       ...
       FROM transactions WHERE book_id = ?${dateFilter}`,
      params
    );
...
app.get('/api/books/:bookId/merchants', verifyToken, async (req, res) => {
  try {
    ...
    const [merchants] = await connection.query(
      'SELECT * FROM merchants WHERE book_id = ? ORDER BY name',
      [bookId]
    );
    connection.release();
    res.json(merchants);
...
app.post('/api/books/:bookId/merchants', verifyToken, async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [result] = await connection.query(
      'INSERT INTO merchants (book_id, name, category_id) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE category_id = VALUES(category_id)',
      [bookId, name, categoryId || null]
    );
    connection.release();
