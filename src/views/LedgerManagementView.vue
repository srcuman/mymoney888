  ...
  <div class="space-y-8">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      <div class="flex justify-between items-center mb-4">
        ...
        <button @click="showAddLedgerModal = true" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary dark:bg-blue-600 dark:hover:bg-blue-700">
          添加账套
        </button>
      </div>
        ...
        <div v-for="ledger in ledgers" :key="ledger.id" class="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-md">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400 dark:text-gray-500">创建时间: {{ ledger.createdAt }}</p>
            </div>
          </div>
    ...
    <!-- 添加/编辑账套模态框 -->
    <div v-if="showAddLedgerModal || showEditLedgerModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md">
        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">{{ showEditLedgerModal ? '编辑账套' : '添加账套' }}</h3>
        <form @submit.prevent="saveLedger" class="space-y-4">
          ...
          <div class="flex justify-end space-x-3">
            <button type="button" @click="showAddLedgerModal = false; showEditLedgerModal = false; resetForm()" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500 rounded-md text-sm font-medium">
              取消
            </button>
...
const saveLedger = () => {
  if (showEditLedgerModal.value) {
    ...
    if (index !== -1) {
      ledgers.value[index] = { ...formData.value }
    }
  } else {
    // 添加新账套
    const newLedger = {
