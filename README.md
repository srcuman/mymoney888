# MyMoney888 - 个人记账软件 v3.0

一个功能强大的个人记账软件，支持多维度分类、账套管理、数据导入导出、信用卡管理、贷款管理等功能。

**v3.0 新特性**：支持SQL数据库同步，实现本地数据与云端数据库的双向同步，确保数据安全和可恢复性。

## 功能特点

### 核心功能
- ✅ 交易记账（收入、支出、转账）
- ✅ 账户管理（支持多种账户类型）
- ✅ 多维度分类（多级分类结构）
- ✅ **账套切换**（一个账号下多个独立账套）
- ✅ **数据管理**（导入导出独立模块）
- ✅ 数据导入（支付宝、微信、CSV）
- ✅ 数据导出（CSV格式）
- ✅ 统计分析（饼图、同比环比分析）
- ✅ 年度回顾
- ✅ 金额计算器
- ✅ 管理员功能（仅限账号管理）
- ✅ **日志记录**（运行日志记录和查看）
- ✅ **数据同步**（v3.0新增：本地数据与SQL数据库双向同步）

### 信用卡管理（v1.7.0新增）
- ✅ 信用卡账户管理
- ✅ 账单日、还款日设置
- ✅ 信用额度管理
- ✅ 本月账单统计
- ✅ 近期还款提醒
- ✅ **分期记账**：支持分期模板、分期名称、多期数选择
- ✅ **账单详情**：按账期查看交易明细
- ✅ **消费分析**：
  - 月份分析（消费金额、交易笔数）
  - 账期分析（账单金额、还款日期、还款状态）
  - 分类分析（消费占比可视化）

### 贷款管理（v1.7.0新增）
- ✅ 多类型贷款支持（房贷、车贷、个人贷款、商业贷款、助学贷款、其他）
- ✅ 贷款信息管理（金额、利率、期限、还款日）
- ✅ 等额本息还款计算
- ✅ 还款进度追踪（可视化进度条）
- ✅ 还款记录管理
- ✅ **智能还款提醒**：
  - 3天内到期：红色警告
  - 7天内到期：黄色提醒
  - 其他：蓝色提示
- ✅ 贷款统计分析（总额、已还、剩余本金、剩余期数）

### 技术特点
- 🚀 基于Vue 3 + Tailwind CSS
- 📦 Docker容器化部署
- 🔒 本地数据存储（LocalStorage）
- 🗄️ SQL数据库支持（v3.0新增：MySQL/MariaDB）
- 🔄 双向数据同步（v3.0新增：智能冲突解决）
- 📱 响应式设计
- 🌐 支持在线升级
- 📝 运行日志记录（便于问题诊断）

## 快速开始

### 方法一：Docker部署（推荐）

#### 使用内置MySQL数据库

```bash
# 克隆仓库
git clone https://github.com/yourusername/mymoney888.git
cd mymoney888

# 复制环境变量模板
cp .env.example .env

# 启动服务（包含MySQL数据库）
docker-compose --profile internal up -d

# 访问 http://localhost:8888
```

#### 使用外部MySQL数据库

```bash
# 克隆仓库
git clone https://github.com/yourusername/mymoney888.git
cd mymoney888

# 复制环境变量模板
cp .env.example .env

# 编辑.env文件，配置外部数据库连接信息
# DB_HOST=your-mysql-host
# DB_PORT=3306
# DB_USER=your-mysql-user
# DB_PASSWORD=your-mysql-password
# DB_NAME=mymoney888

# 启动服务（不包含MySQL）
docker-compose up -d

# 访问 http://localhost:8888
```

**详细部署说明请参考**：[DEPLOY.md](DEPLOY.md)

### 方法二：直接运行

```bash
# 克隆仓库
git clone https://github.com/yourusername/mymoney888.git
cd mymoney888

# 安装依赖
npm install

# 复制环境变量模板
cp .env.example .env

# 编辑.env文件，配置数据库连接信息
# DB_HOST=localhost
# DB_PORT=3306
# DB_USER=root
# DB_PASSWORD=your_password
# DB_NAME=mymoney888

# 初始化数据库
mysql -u root -p < database/init-db.sql

# 启动服务器
node dist/server.mjs

# 访问 http://localhost:8888
```

### 数据库配置（v3.0）

v3.0版本支持SQL数据库，实现本地数据与云端数据库的双向同步。

**配置方式**：

1. **使用环境变量**（推荐）：
   复制 `.env.example` 为 `.env`，然后编辑配置：
   ```env
   DB_HOST=localhost
   DB_PORT=3306
   DB_USER=root
   DB_PASSWORD=your_password
   DB_NAME=mymoney888
   ```

2. **自动初始化**：
   - 使用Docker部署时，应用会在启动时自动初始化数据库结构
   - 直接运行时，需要手动执行 `database/init-db.sql` 脚本

**数据库功能**：
- ✅ 支持MySQL 8.0+
- ✅ 自动创建表结构
- ✅ 双向数据同步
- ✅ 智能冲突解决
- ✅ 同步日志记录

**详细说明请参考**：[DATABASE_DESIGN.md](DATABASE_DESIGN.md)
npm run server
```

## 账户信息

- **首次注册**：需要输入管理员注册码 `admin123` 来注册为管理员
- **普通用户注册**：在已有管理员后，可直接注册为普通用户
- **管理员功能**：管理员可以管理所有用户账户，包括重置密码

## 项目结构

```
mymoney888/
├── dist/              # 生产构建文件
│   ├── app.js         # 主应用代码
│   ├── index.html     # 主页面
│   └── server.mjs     # 服务器脚本
├── src/               # 源代码（开发用）
│   ├── router/        # 路由配置
│   ├── views/         # 页面组件
│   ├── services/      # 服务模块（v3.0新增）
│   │   ├── sync-service.js    # 数据同步服务
│   │   └── api-service.js     # API服务
│   ├── stores/        # 状态管理（v3.0新增）
│   │   └── sync.js           # 同步状态Store
│   ├── App.vue        # 根组件
│   ├── main.js        # 应用入口
│   └── style.css      # 样式文件
├── database/          # 数据库脚本（v3.0新增）
│   └── init-db.sql    # 数据库初始化脚本
├── Dockerfile         # Docker构建文件
├── docker-compose.yml # Docker Compose配置
├── package.json       # 项目配置
├── .env.example       # 环境变量示例（v3.0新增）
└── README.md          # 项目说明
```

## 功能使用指南

### 1. 记账功能
- 在首页点击记账按钮
- 选择交易类型（收入/支出/转账）
- 输入金额（支持计算器功能）
- 选择分类、账户等信息
- 交易时间默认为当前时间（Asia/Shanghai时区）
- 点击保存

### 2. 信用卡分期记账
- 记账时选择信用卡账户
- 勾选"使用分期"
- 可选择已保存的分期模板或手动设置
- 输入分期名称（如商品名称）
- 保存后可选择"保存为模板"供下次使用

### 3. 信用卡管理
- 点击顶部导航"信用卡"进入管理页面
- 点击"添加信用卡"添加新卡
- 设置账单日、还款日、信用额度等信息
- 点击"查看账单"查看具体账期明细
- 点击"分析"查看消费分析报表

### 4. 贷款管理
- 点击顶部导航"贷款"进入管理页面
- 点击"添加贷款"记录贷款信息
- 设置贷款类型、金额、利率、期限等
- 点击"记录还款"记录每期还款
- 查看"近期还款提醒"了解即将到期的还款

### 5. 账套切换
- 点击导航栏右侧的账套名称
- 在下拉菜单中选择要切换的账套
- 或点击「新建账套」创建新账套
- 切换账套后页面会自动刷新加载新账套数据
- 各账套数据完全隔离，互不干扰

### 6. 数据管理
- 点击顶部导航「数据管理」进入数据管理页面
- **导出数据**：将所有交易记录导出为CSV格式
- **导入数据**：支持多种格式导入
  - JSON格式（完整数据备份）
  - 支付宝账单（CSV）
  - 微信账单（CSV）
  - 通用CSV格式（可下载样板）

### 7. 数据导入（账单导入）
- 支持支付宝账单导入
- 支持微信账单导入
- 支持通用CSV导入

### 8. 统计分析
- 支持多种快捷查询条件
- 支持同比、环比分析
- 支持饼图展示

### 9. 年度回顾
- 查看年度财务总结
- 智能洞察和建议

### 10. 日志查看
- 点击顶部导航"日志"进入日志页面
- 查看系统运行日志
- 支持按级别筛选、搜索、导出

### 11. 数据同步（v3.0新增）
- **自动同步**：系统会自动检测网络状态，在线时自动同步数据
- **手动同步**：点击导航栏的同步按钮手动触发同步
- **同步状态**：实时显示同步状态和最后同步时间
- **冲突解决**：当本地和服务器数据冲突时，提供智能冲突解决策略
- **同步历史**：查看所有同步操作的历史记录
- **离线支持**：离线时数据保存在本地，联网后自动同步

**同步策略**：
- 优先使用最新修改时间的数据
- 支持手动选择本地或服务器数据
- 支持合并冲突数据
- 同步失败时保留本地数据，下次重试

## 在线升级

当有新版本时，系统会自动检测并提示升级。点击升级按钮即可完成更新。

## 版本历史

### v3.0.0 (2026-03-28)
- ✨ 新增SQL数据库支持（MySQL/MariaDB）
- ✨ 新增本地数据与数据库双向同步功能
- ✨ 新增智能冲突解决机制
- ✨ 新增同步状态监控和历史记录
- ✨ 新增离线数据缓存和自动同步
- ✨ 新增数据库初始化脚本和存储过程
- ✨ 新增数据同步服务模块（sync-service.js）
- ✨ 新增API服务模块（api-service.js）
- ✨ 新增同步状态管理Store（sync.js）
- 🎨 优化数据持久化机制
- 🎨 优化网络状态检测
- 📚 新增数据库设计文档

### v1.9.0 (2026-03-07)
- ✨ 优化分期交易显示逻辑（交易明细显示总额，信用卡账单显示每期）
- ✨ 新增分期唯一标识（自动生成分期组ID）
- ✨ 新增分期详情查看功能（点击分期ID查看完整明细）
- 🎨 优化信用卡账单界面（添加分期ID列）
- 🐛 修复分期详情模态框重复问题

### v1.8.0 (2025-03-07)
- ✨ 新增账套切换功能（一个账号下多个独立账套）
- ✨ 新增数据管理独立模块（导入导出功能）
- 🔒 限制管理员仅限账号管理功能
- 🎨 优化导航栏布局（添加账套切换下拉菜单）

### v1.7.1 (2025-03-07)
- 🐛 修复记账功能中的分期交易记录问题
- ✨ 实现信用卡与账户系统的联动（还款记录为交易）
- ✨ 实现贷款与交易系统的联动（还款记录为交易）
- ✨ 恢复CSV样板下载功能
- 🎨 优化信用卡还款提醒（显示天数和快捷还款按钮）
- 🎨 优化贷款还款记录（自动创建交易记录）

### v1.7.0 (2025-03-07)
- ✨ 新增贷款管理功能
- ✨ 新增信用卡分期模板功能
- ✨ 新增信用卡消费分析（月份、账期、分类）
- ✨ 新增运行日志记录功能
- 🎨 优化交易时间录入（合并日期时间选择器）
- 🎨 优化信用卡管理界面

### v1.6.0
- ✨ 新增信用卡管理功能
- ✨ 新增分期记账功能
- ✨ 新增年度回顾功能
- ✨ 新增金额计算器
- ✨ 新增管理员功能
- ✨ 新增在线升级功能

## 技术栈

- **前端**：Vue 3, Tailwind CSS, Chart.js
- **后端**：Node.js（轻量级HTTP服务器）
- **存储**：LocalStorage（本地数据持久化）
- **部署**：Docker, Docker Compose

## 开发指南

### 开发环境搭建

```bash
# 安装依赖
npm install

# 启动开发服务器
npm run dev

# 构建生产版本
npm run build
```

### 项目配置

- **端口**：8888
- **数据存储**：LocalStorage（按账套隔离）
- **默认账套**：default
- **时区**：Asia/Shanghai

## 注意事项

1. 数据存储在浏览器本地，建议定期导出备份
2. 支持主流浏览器：Chrome、Firefox、Safari、Edge
3. 推荐使用Docker部署以获得最佳体验
4. 信用卡和贷款数据按账套隔离存储

## 许可证

MIT License

## 贡献

欢迎提交Issue和Pull Request！

---

**MyMoney888** - 让记账更简单，让生活更美好！