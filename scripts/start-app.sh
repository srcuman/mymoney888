...
if [ -n "$DB_HOST" ] && [ -n "$DB_USER" ] && [ -n "$DB_PASSWORD" ] && [ -n "$DB_NAME" ]; then
    ...
    while [ $attempt -lt $max_attempts ]; do
        ...
        # 捕获mysql命令的详细错误信息
        ERROR_OUTPUT=$(MYSQL_PWD="$DB_PASSWORD" mysql -h"$DB_HOST" -P"${DB_PORT:-3306}" -u"$DB_USER" --skip-ssl -e "SELECT 1" 2>&1)
        MYSQL_EXIT_CODE=$?
    ...
    if [ $attempt -eq $max_attempts ]; then
        ...
        # 检查数据库是否存在
        echo "检查数据库是否存在..."
        DB_EXISTS=$(MYSQL_PWD="$DB_PASSWORD" mysql -h"$DB_HOST" -P"${DB_PORT:-3306}" -u"$DB_USER" --skip-ssl -e "SHOW DATABASES LIKE '$DB_NAME'" 2>&1 | grep "$DB_NAME" || true)
        
        if [ -z "$DB_EXISTS" ]; then
            echo "数据库 $DB_NAME 不存在，正在创建..."
            
            # 捕获创建数据库的错误
            CREATE_DB_ERROR=$(MYSQL_PWD="$DB_PASSWORD" mysql -h"$DB_HOST" -P"${DB_PORT:-3306}" -u"$DB_USER" --skip-ssl -e "CREATE DATABASE IF NOT EXISTS $DB_NAME CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" 2>&1)
        ...
        # 检查是否需要初始化表结构
        echo "检查表结构..."
        TABLE_COUNT=$(MYSQL_PWD="$DB_PASSWORD" mysql -h"$DB_HOST" -P"${DB_PORT:-3306}" -u"$DB_USER" --skip-ssl "$DB_NAME" -e "SHOW TABLES" 2>&1 | wc -l)
        ...
        if [ "$TABLE_COUNT" -le 1 ] || [ "$TABLES_EXIST" -lt 18 ]; then
            ...
            if [ -f "/app/database/init-db.sql" ]; then
                ...
                # 捕获SQL执行的错误
                SQL_ERROR=$(MYSQL_PWD="$DB_PASSWORD" mysql -h"$DB_HOST" -P"${DB_PORT:-3306}" -u"$DB_USER" --skip-ssl "$DB_NAME" < "/app/database/init-db.sql" 2>&1)
                SQL_EXIT_CODE=$?
