...
- **驱动更换**: mysql2 → pg
- **语法适配**: AUTO_INCREMENT → SERIAL, TINYINT → BOOLEAN, JSON → JSONB
- **功能增强**: 增加投资模块表结构（investment_holdings, nav_history, investment_transfers）
...
3. [索引设计](#索引设计)
4. [视图设计](#视图设计)
5. [触发器设计](#触发器设计) *(已移除，需在应用层实现)*
6. [存储过程设计](#存储过程设计) *(已移除，需在应用层实现)*
7. [数据同步机制](#数据同步机制)
8. [数据完整性约束](#数据完整性约束)
9. [性能优化建议](#性能优化建议)
